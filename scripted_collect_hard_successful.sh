...
GRASP_TEST_OBJECTS='square_rod_embellishment grill_trash_can sack_vase two_handled_vase thick_wood_chair curved_handle_cup baseball_cap elliptical_capsule'

for obj in $GRASP_TEST_OBJECTS; do
    CUDA_VISIBLE_DEVICES=1 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp -a grasp_success -n 1000 -t 15 -d /media/3tb/chet/robo_exp/trajs/grasp_hard_test_successful/${obj} --target-object ${obj} -p 10
done

for obj in $GRASP_TRAIN_OBJECTS; do
    CUDA_VISIBLE_DEVICES=1 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp -a grasp_success -n 1000 -t 15 -d /media/3tb/chet/robo_exp/trajs/grasp_hard_train_successful/${obj} --target-object ${obj} -p 10
done