...
GRASP_TRAIN_OBJECTS='fountain_vase circular_table smushed_dumbbell narrow_tray colunnade_top stalagcite_chunk bongo_drum_bowl pacifier_vase beehive_funnel crooked_lid_trash_can toilet_bowl pepsi_bottle tongue_chair modern_canoe pear_ringed_vase short_handle_cup bullet_vase glass_half_gallon flat_bottom_sack_vase trapezoidal_bin vintage_canoe bathtub flowery_half_donut t_cup cookie_circular_lidless_tin box_sofa two_layered_lampshade conic_bin jar bunsen_burner long_vase ringed_cup_oversized_base aero_cylinder'
...
GRASP_TEST_OBJECTS='square_rod_embellishment curved_handle_cup'

for obj in $GRASP_TEST_OBJECTS; do
    CUDA_VISIBLE_DEVICES=3 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp -a grasp_success --save-all -n 1000 -t 15 -d /media/3tb/chet/robo_exp/grasp_hard_test_all/${obj} --target-object ${obj} -p 10
done

for obj in $GRASP_TRAIN_OBJECTS; do
    CUDA_VISIBLE_DEVICES=3 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp -a grasp_success --save-all -n 1000 -t 15 -d /media/3tb/chet/robo_exp/grasp_hard_train_all/${obj} --target-object ${obj} -p 10
done