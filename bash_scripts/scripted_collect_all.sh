...
for obj in $GRASP_TEST_OBJECTS; do
    CUDA_VISIBLE_DEVICES=$gpu python scripts/scripted_collect_parallel.py --save-all -e Widow250Grasp${env}_${obj}-v0 -pl rotate_grasp -a grasp_success -n $num_trajs -t 50 -d $dir/grasp_${env,,}_test_all/${obj} --target-object ${obj} -p 10
done

for obj in $GRASP_TRAIN_OBJECTS; do
    echo CUDA_VISIBLE_DEVICES=$gpu python scripts/scripted_collect_parallel.py --save-all -e Widow250Grasp${env}_${obj}-v0 -pl rotate_grasp -a grasp_success -n $num_trajs -t 50 -d $dir/grasp_${env,,}_train_all/${obj} --target-object ${obj} -p 10
done