...
GRASP_TRAIN_OBJECTS='flat_bottom_sack_vase'

for obj in $GRASP_TRAIN_OBJECTS; do
    CUDA_VISIBLE_DEVICES=3 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp_lift -a grasp_success --save-all -n 200 -t 25 -d /media/3tb/chet/robo_exp/trajs/lift_grasp_hard_train_all/${obj} --target-object ${obj} -p 10
done

# for obj in $GRASP_TEST_OBJECTS; do
#     CUDA_VISIBLE_DEVICES=3 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp_lift -a grasp_success --save-all -n 100 -t 25 -d /media/3tb/chet/robo_exp/trajs/lift_grasp_hard_test_all/${obj} --target-object ${obj} -p 10
# done