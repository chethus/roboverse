...
GRASP_TRAIN_OBJECTS='jar bunsen_burner long_vase ringed_cup_oversized_base aero_cylinder'
...
for obj in $GRASP_TRAIN_OBJECTS; do
    CUDA_VISIBLE_DEVICES=2 python scripts/scripted_collect_parallel.py -e Widow250GraspHard_${obj}-v0 -pl grasp -a grasp_success -n 40 -t 15 -d /media/3tb/chet/robo_exp/grasp_hard_trajs_check/${obj} --target-object ${obj} -p 10
done
